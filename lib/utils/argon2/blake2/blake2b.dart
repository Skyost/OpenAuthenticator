...
import 'package:open_authenticator/utils/argon2/blake2/ufixnum.dart';

class Blake2bDigest extends Digest {
  ...
  final _internalState = Register64List(16); // In the Blake2b paper it is called: v
  Register64List? _chainValue; // state vector, in the Blake2b paper it is called: h

  final _t0 = Register64(); // holds last significant bits, counter (counts bytes)
  final _t1 = Register64(); // counter: Length up to 2^128 are supported
  final _f0 = Register64(); // finalization flag, for last block: ~0L

  Blake2bDigest({int digestSize = 64, Uint8List? key, Uint8List? salt, Uint8List? personalization}) {
  ...
  void init() {
    if (_chainValue == null) {
      ...
      if (_personalization != null) {
        _chainValue![6].xor(Register64()..unpack(_personalization, 0, Endian.little));
        _chainValue![7].xor(Register64()..unpack(_personalization, 8, Endian.little));
      }
    }
  }
  ...
  void _initializeInternalState() {
    ...
    _internalState.setRange(_chainValue!.length, _chainValue!.length + 4, _blake2bIV);
    _internalState[12]
      ..set(_t0)
      ..xor(_blake2bIV[4]);
    _internalState[13]
  ...
  void _compress(Uint8List? message, int messagePos) {
    ...
    for (var round = 0; round < _rounds; ++round) {
      ...
      G(_m[_blake2bSigma[round][10]], _m[_blake2bSigma[round][11]], 1, 6, 11, 12);
      G(_m[_blake2bSigma[round][12]], _m[_blake2bSigma[round][13]], 2, 7, 8, 13);
      G(_m[_blake2bSigma[round][14]], _m[_blake2bSigma[round][15]], 3, 4, 9, 14);
    }
